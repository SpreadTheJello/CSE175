...
def uniform_cost_search(problem, repeat_check=False):
    ...
    # Since we're doing uniform cost search, we use a priority queue that sorts by g(n) cost
    nodeFrontier = Frontier(startNode, sort_by='g')
        ...
        for node in reachableNodes:
            ...
            if repeat_check == True:
                if node in visitedNodes:
                    ...
                    if nodeFrontier.contains(node) and (nodeFrontier[node] > node.value(sort_by="g")):
